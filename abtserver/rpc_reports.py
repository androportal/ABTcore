...
class reports(xmlrpc.XMLRPC):
	...
	def xmlrpc_getGrossTrialBalance(self,queryParams,client_id):
		...
		* Purpose:
			...
			- instead of the current balance it provides the total Dr 
			  and total Cr for all accounts.
	...
	def xmlrpc_getProjectStatementReport(self,queryParams,client_id):
		...
		* Input:
			- [projectname,financial_fromdate,fromdate,todate]
		
		* Output:
			- list of list [serialno,accountname,groupname,totalDr,totalCr]
		"""
		account = rpc_account.account()
	...
	def xmlrpc_getProjectStatement(self,queryParams,client_id):
		...
		* Input:
			- [projectname,accountname,financial_fromdate,fromdate,todate]
	...
	def xmlrpc_getBalancesheetDisplay(self,queryParams,client_id):
		...
		* Output:
			- if balancesheet type is ``Conventional Balance Sheet``
				+ list of list [groupname,debit amount,credit amount,total amount])
			- else 
				+ list of list [Particulars,debit amount,credit amount,Amount,Amount]	
		"""
		if (queryParams[5] == None):
			profitloss = self.xmlrpc_getProfitLossDisplay(queryParams,client_id)
			
			connection = dbconnect.engines[client_id].connect()
			Session = dbconnect.session(bind=connection)
			result = Session.query(dbconnect.ProfitLoss.net_flag,dbconnect.ProfitLoss.opening_balance).first()
			Session.close()
			...
			if result == None:
				...
				Net_Flag = result[0]
				Net_Opening = float(result[1])
		
			Flag = profitloss[0]
		
			if (Net_Flag == "netProfit" and Flag =="netProfit"):
				final_net_value = float(profitloss[1])+Net_Opening
				final_flag = "netProfit"
				...
				if(float(profitloss[1]) > Net_Opening):
					...
					final_net_value = Net_Opening - float(profitloss[1])
					final_flag = "netProfit"
			elif (Net_Flag == "netLoss" and Flag =="netProfit"):
				if(float(profitloss[1]) > Net_Opening):
					final_net_value = float(profitloss[1]) - Net_Opening
					final_flag = "netProfit"
				else :
					final_net_value = Net_Opening - float(profitloss[1])
					final_flag = "netLoss"
			return 	[final_flag,final_net_value,Net_Opening,Net_Flag]
		else:
			...
			pnlcr = float(totalCr) + final_net_value
		
			pnldr = float(totalDr) + final_net_value
			
			pnl1 = '%.2f'%float(pnlcr)
			pnl2 = '%.2f'%float(pnldr)
			if final_flag =="netProfit":
			...
			if balancesheet_type == "Conventional Balance Sheet": 
				...
				if (final_flag != "netLoss"):
					...
					if (final_net_value != "0.00"):
						corpuslist.append([flag,"",'%.2f'%(final_net_value)])	
						corpuslist.append(["            Opening "+Net_Flag,'%.2f'%(Net_Opening),""])	
						corpuslist.append(["            Current "+Flag,'%.2f'%(float(profitloss[1])),""])
						Lcount = Lcount+3
				...
				if (final_flag == "netLoss"):
					...
					if (final_net_value != "0.00"):	
						assetslist.append([flag,"",'%.2f'%(final_net_value)])# net surplus or net profit
						assetslist.append(["            Opening "+Net_Flag,'%.2f'%(Net_Opening),""])	
						assetslist.append(["            Current "+Flag,'%.2f'%(float(profitloss[1])),""])
						Rcount = Rcount+3
				#else:
					#assetslist.append(["","","",""])	
					#Rcount = Rcount+1
			...
			if balancesheet_type == "Sources and Application of Funds": 
				...
				if (final_net_value != "0.00"):			
					...
					if (final_flag == "netLoss"):
						...
						LeftList.append(["              "+flag,'%.2f'%(final_net_value),"",""])	
						LeftList.append(["            Opening "+Net_Flag,'%.2f'%(Net_Opening),"",""])	
						LeftList.append(["            Current "+Flag,'%.2f'%(float(profitloss[1])),"",""])
					
						amount = float(tot_reserves) - float(final_net_value)
				...
				if (tot_miscellaneous != "0.00"):		
					...
					if (final_flag == "netLoss"):
						amount = float(tot_reserves) - float(final_net_value) - float(tot_miscellaneous)
					else:
						amount = float(tot_reserves) + float(final_net_value) - float(tot_miscellaneous)
				...
				if (difamount != "0.00"):
					if (final_flag == "netLoss"):
						amount= float(tot_capital) + \
							float(tot_loanlia) + \
							(float(tot_reserves) - float(final_net_value) - float(tot_miscellaneous))
					else:
						amount= float(tot_capital) + \
							float(tot_loanlia) + \
							(float(tot_reserves) + float(final_net_value) - float(tot_miscellaneous))
	...
	def xmlrpc_getProfitLossReport(self,queryParams,client_id):
		...
		* Note: 
			- profit and loss is always calculated from 
		...
		* Output: 
			- returns a grid of 4 columns and number of rows 
			  depending on number of accounts.
			- List of list [serial no,groupcode,accountname,amount,balancetype]
		"""
		grpCodes = [4,5,7,8]
		profitloss = []
		srno = 1
		...
		if (total_dirInc_balances > total_dirExp_balances):
			...
			if(totalnetprofit > total_indirExp_balances):
				...
				#Session.add(dbconnect.ProfitLoss("netProfit",'%.2f'%(float(netProfit))))
				#Session.commit()
				...
				#Session.add(dbconnect.ProfitLoss("netLoss",('%.2f'%(float(netLoss)))))
				#Session.commit()
	...
	def xmlrpc_getProfitLossDisplay(self,queryParams,client_id):	
		...
		* Output:
			- returns 3 dimentional list [[["","Particulars","Amount"]]]
			- list of list contan list for DE DI IE and II
		"""
	...
	def xmlrpc_setBankRecon(self,queryParams,client_id):
		...
		* Output:
		...
		if reconcode == None:
			...
			reconcode = reconcode + 1
		accountcode = accObj.xmlrpc_getAccountCodeByAccountName([queryParams[2]], client_id)	
		result = Session.query(dbconnect.BankRecon).\
					filter(and_(dbconnect.BankRecon.accountcode == accountcode,\
		...
		if result == None:
			if queryParams[3] == 0:
				...
				Session.add(dbconnect.BankRecon(reconcode,queryParams[0],reffdate,accountcode,\
	...
	def xmlrpc_getClearanceDate(self,queryParams,client_id):
		...
		* Output:
		...
		Session = dbconnect.session(bind=connection)
		result = Session.query(dbconnect.BankRecon).\
				filter(and_(dbconnect.BankRecon.accountcode==queryParams[0],\
	...
	def xmlrpc_deleteClearedRecon(self,queryParams,client_id):
		...
		* Input:
			- [accountcode,vouchercode,todate]
		
		* Output:
			- If accountcode, vouchercode matches and clearencedate less than 
			  input clearencedate then delete clearRecon and return True
			  else return False
		"""
	...
	def xmlrpc_updateBankRecon(self,queryParams, flags, client_id):
		...
		* Purpose:
			...
			- Then, on every iteration it calls following functions 
				...
				+ if cleared_tran_flag is True, it calls ``getReconLedger`` function to 
